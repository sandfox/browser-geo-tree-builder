...
var  K = 2; // I only require a 2d k-d tree & currently I'm too lazy to calculate bounding hyperrectangles, 2d ones will do just fine.
  ...
  function getHaversineDistance(x1, y1, x2, y2) {
    ...
    var  dLat = (b.y-a.y);
    var  dLon = (b.x-a.x);
    var  c = Math.sin(dLat/2) * Math.sin(dLat/2) + Math.cos(a.y) * Math.cos(b.y) * Math.sin(dLon/2) * Math.sin(dLon/2);
    var  distance = 6371 * 2 * Math.atan2(Math.sqrt(c), Math.sqrt(1-c));
    return distance;
